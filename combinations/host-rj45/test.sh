...
exteid=`imunes$legacy -b -e horj ext.imn | tail -1 | cut -d' ' -f4`
startCheck "$exteid"
...
if [ $? -eq 0 ]; then
    netDump host1@$eid eth0 icmp
    if [ $? -eq 0 ]; then
	pingCheck host1@$eid 10.0.0.21 2
	if [ $? -eq 0 ]; then
	    sleep 2
	    readDump host1@$eid eth0
	    if [ $? -ne 0 ]; then
...
if [ $? -ne 0 ]; then
    netDump host1@$eid eth1 icmp
    if [ $? -eq 0 ]; then
	pingCheck host1@$eid 10.0.1.21 2
	if [ $? -eq 0 ]; then
	    sleep 2
	    readDump host1@$eid eth1
	    if [ $? -ne 0 ]; then
		echo "ERROR: readDump failed"
		err=1
	    fi
