...
if isOSlinux; then
    ...
    himage $sun /usr/local/libexec/ipsec/charon &
	himage -b $sun ifconfig eth0 inet6 -ifdisabled
fi
...
for i in `seq 1 $steps`
do
    himage -nt $moon swanctl --list-conns 2>&1 | grep ^[[:space:]]*net64-net64: >/dev/null
    er1=$?
    himage -nt $sun swanctl --list-conns 2>&1 | grep ^[[:space:]]*net64-net64: >/dev/null
    er2=$?
