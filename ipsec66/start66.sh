...
hcp moon66_swanctl.conf ${moon}:${DIR}/swanctl.conf
hcp sun66_swanctl.conf ${sun}:${DIR}/swanctl.conf
...
for i in `seq 1 $steps`
do
    himage -nt $moon swanctl --list-conns 2>&1 | grep ^[[:space:]]*net66-net66: >/dev/null
    er1=$?
    himage -nt $sun swanctl --list-conns 2>&1 | grep ^[[:space:]]*net66-net66: >/dev/null
    er2=$?
